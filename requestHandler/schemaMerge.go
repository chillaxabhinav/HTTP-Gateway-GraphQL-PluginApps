...
// Will run after mapper is ready
func SchemaMerge() (*ast.Schema, error) {

	global := GetSchemaRegistry()

	var myAstSlices []*ast.Schema

	for _, val := range *global {
		myAstSlices = append(myAstSlices, val.Introspection)
	}

	mergedSchema, err := mergeSchemas(myAstSlices)
